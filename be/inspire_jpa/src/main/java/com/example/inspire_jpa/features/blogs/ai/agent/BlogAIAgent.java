...
public class BlogAIAgent {
    ...
    private final BlogAITool blogAITool;

    public String generate(Map<String, Object> map) {
        System.out.println("debug >>>> blog AI agent genearate");
        ...
        System.out.println("debug >>>> blog agent genearate result : ");
    ...
    public BlogResponseDTO insert(BlogRequestDTO request) {
        ...
        String systemPrompt = """
            ...
            - saveBlog 호출 후 최종 응답은 저장된 BlogResponseDTO를 JSON으로만 반환한다.
            </규칙>
        """;

        String userPrompt="""
            아래 요청사항에 따라 기존 글과 중복되지 않는 블로그 글을 작성하고 저장해 줘.
            category  : %s , 
            title   :  %s ,
            content :  %s ,
            email   :  %s 
        ...
        BlogResponseDTO result = blogChatClient.prompt()
                ...
                System.out.println("debug >>>> blog agent insert result : ");
                System.out.println(result);
                return result;
    }
}
