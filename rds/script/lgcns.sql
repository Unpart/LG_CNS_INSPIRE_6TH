...
-- ON DELETE NO OPTION, ON UPDATE NO OPTIONNO OPTION
...
INSERT INTO TB_EMP(EMP_ID, EMP_NAME, HIRE_DATE, DEPT_ID, JOB_ID)
VALUES('400','JSLIM', NULL, '20', 'J1'),
		('500','JSLIM', NULL, '20', 'J1'),
		('600','JSLIM', NULL, '20', 'J1');
...
-- 	ADD, DROP CONSTRAINT
-- 	ADD COLUMN
-- 	CHANGE COLUMN OLD NEW INT -- 이름과 타입변경
-- 	MODIFY COLUMN OLD INT -- 타입변경
-- 	DROP COLUMN COLUMN_NAME -- 타입변경
-- RENAME -- 테이블 이름 변경
ALTER TABLE TB_EMPLOYEE
ADD CONSTRAINT PRIMARY KEY(EMP_ID);

-- Q) JOB_ID, DEPT_ID 컬럼에 ALTER 명렁어 이용해서 NOT NULL, FOREIGN KEY 추가
...
-- Q6)
ALTER TABLE tb_category
CHANGE COLUMN NAME CATEGORY_NAME VARCHAR(20) NOT NULL;
...
-- UPDATE, INSERT, DELETE, TRANSACTION(COMMIT, ROLLBACK)

-- UPDATE : 테이블에 포함된 기존 데이터를 수정(생 수는 달라지지 않음)
/*
UPDATE TABLE_NAME
SET	 COLUMN_NAME = { VALUE | SUBQUERY }, COLUMN_NAME = { VALUE | SUBQUERY }
...
-- Q) 해외영업2팀의 사원의 보너스비율(BONUS_PCT)을 0.3 수정하고 싶다면?
UPDATE employee
SET BONUS_PCT = 0.3
...
SELECT	PREVIOUS VALUE FOR SEQ_TEST ; -- XXXX
